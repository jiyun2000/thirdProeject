    ...
    return Scaffold(
        ...
        body: Column(
          children: [
            Text('환영합니다 '),
            Center(
              //출퇴근 gps 연동 버튼 넣고 시간 띄우기
              child: Text('출퇴근 연동시키기'),
            ),
            FutureBuilder(
              //오늘 개인 일정
              future:
                  emp.EmpScheDio().readEmpTodo(1, DateTime.parse(dayFormat)),
            ...
            FutureBuilder(
              ...
              future: dept.DeptScheDio()
                  .readDeptTodo(1, 1, DateTime.parse(dayFormat)),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return Center(child: CircularProgressIndicator());
                } else if (snapshot.hasError) {
                    ...
                    itemBuilder: (context, index) {
                      return Card(
                        ...
                        child: ListTile(
                          contentPadding: EdgeInsets.all(16),
                          leading: Icon(Icons.circle),
                          title: Text(deptSchedule[index].scheduleText,
                              style: TextStyle(fontSize: 16)),
                        ),
                      );
                    },
